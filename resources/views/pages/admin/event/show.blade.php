                ...
                <table class="table">
                    ...
                    <tbody>
                        @forelse ($tickets as $index => $ticket)
                            <tr>
                                <th>{{ $index + 1 }}</th>
                                <td>{{ $ticket->tipeTicket->nama ?? '-' }}</td>
                                <td>{{ $ticket->harga }}</td>
                                <td>{{ $ticket->stok }}</td>
                                <td>
                                    <button class="btn btn-sm btn-primary mr-2" onclick="openEditModal(this)"
                                        data-id="{{ $ticket->id }}" data-tipe-ticket-id="{{ $ticket->tipe_ticket_id }}"
    ...
    <dialog id="add_ticket_modal" class="modal">
        <form method="POST" action="{{ route('admin.tickets.store') }}" class="modal-box">
            ...
            <div class="form-control mb-4">
                ...
                <select name="tipe_ticket_id" class="select select-bordered w-full" required>
                    <option value="" disabled selected>Pilih Tipe Ticket</option>
                    @foreach($tipeTickets as $tipe)
    ...
    <dialog id="edit_ticket_modal" class="modal">
        <form method="POST" class="modal-box">
            ...
            <div class="form-control mb-4">
                ...
                <select name="tipe_ticket_id" id="edit_tipe_ticket_id" class="select select-bordered w-full" required>
                    <option value="" disabled selected>Pilih Tipe Ticket</option>
                    @foreach($tipeTickets as $tipe)
                        <option value="{{ $tipe->id }}">{{ $tipe->nama }}</option>
                    @endforeach
